...
#: Where the BATCH pipeline writes its finalized reports.  Sequential writes to
#: `sequential.evidence.COMBINED_DIRNAME` ("combined") instead, and passes that
#: through `run(reports_dir=...)`.
...
def run(*, batch_root: str, s3_client=None, skip_upload: bool = False,
        skip_ingest: Optional[bool] = None, requests_mod=None,
        reports_dir: str = DEFAULT_REPORTS_DIR) -> Dict[str, Any]:
    ...
    `reports_dir` is the subdirectory of `batch_root` holding
    `combined_train_report.json`.  It defaults to `"reports"`, which is where the
    BATCH pipeline writes, so every existing caller is unaffected.  The
    SEQUENTIAL pipeline writes to `"combined"`
    (`sequential.evidence.COMBINED_DIRNAME`), so its caller passes that -- a
    parameter rather than a change to either layout, because both are correct for
    their own pipeline and neither should move to suit this adapter."""
...
def _run_inner(*, batch_root, s3_client, skip_upload, skip_ingest,
               requests_mod, result,
               reports_dir=DEFAULT_REPORTS_DIR) -> Dict[str, Any]:
    # The finalized-report directory differs by pipeline -- `reports/` for Batch,
    # `combined/` for Sequential -- so the caller names it rather than this
    # adapter assuming one.
    report_path = os.path.join(batch_root, reports_dir,
