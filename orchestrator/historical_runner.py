...
def run(
    *,
    ...
    """
    # BATCH ONLY.  Kept as an explicit parameter (rather than dropped) so a
    # caller that still passes mode="sequential" fails with a sentence instead
    # of silently getting a batch run it did not ask for.
    if str(mode or "batch").strip().lower() != "batch":
        log.error("[HISTORICAL] mode=%r is not supported: this repository's "
                  "historical mode runs the BATCH pipeline only.  Drop the "
                  "flag, or run the sequential architecture in the foreground "
                  "with `--mode sequential`.", mode)
        return 2
